...
- **Champion portraits**: pick/ban cards, each team's pick slots, and bans now show the champion's portrait (`img/champions/{id}.png`, generated by you). As long as a champion doesn't have an image yet, its slot simply stays text-only — nothing breaks as images get added over time.
- **New draft screen layout**: your team's picks and the opponent's now flank the center area (turn banner, coach, filters, champion grid), modeled after professional draft tools.
- **Series ban history**: a new block at the bottom of the screen recaps the bans of the current game and every previous game of the same series (BO3/BO5), most recent first.
